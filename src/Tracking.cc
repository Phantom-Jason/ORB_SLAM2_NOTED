    ...
    mpFrameDrawer(pFrameDrawer), mpMapDrawer(pMapDrawer), mpMap(pMap), mnLastRelocFrameId(0)
{
    ...
    if(sensor==System::RGBD)
    {
        ...
        if(fabs(mDepthMapFactor)<1e-5)
            ...
            mDepthMapFactor = 1.0f/mDepthMapFactor;   //  depth = f*baseline/disparity
    }
...
cv::Mat Tracking::GrabImageMonocular(const cv::Mat &im, const double &timestamp)
{
    ...
    if(mState==NOT_INITIALIZED || mState==NO_IMAGES_YET)// 没有成功初始化的前一个状态就是NO_IMAGES_YET 或没有关键帧
    ...
    else// 步骤2：跟踪
    {
        ...
        if(!mbOnlyTracking)
        {
            ...
            if(mState==OK)
            {
                ...
                if(mVelocity.empty() || mCurrentFrame.mnId<mnLastRelocFrameId+2) //刚重定位成功
                {
                    // 将上一帧的位姿作为当前帧的初始位姿
